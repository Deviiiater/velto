...
export default function Home() {
  ...
  const [weather, setWeather] = useState<'sunny' | 'rainy' | 'winter'>('sunny');
  const [timeOfDay, setTimeOfDay] = useState<'morning' | 'afternoon' | 'evening' | 'night'>('evening');
  const [aiOrderBuilderPrompt, setAiOrderBuilderPrompt] = useState('');
  const [aiRecommendedCart, setAiRecommendedCart] = useState<Product[] | null>(null);
  const [showAiCartPopup, setShowAiCartPopup] = useState(false);
  ...
  const handleAiAction = (actionType: 'budget' | 'sick' | 'guests') => {
    let selectedIds: string[] = [];
    let title = '';
    
    if (actionType === 'budget') {
      title = "AI ₹250 Budget Meal Builder";
      selectedIds = ['combo-student', 'c905db42-998f-4cf6-ba9e-1d5be5a248f2', 'b819f71c-32e6-42ee-8e7f-b649d2146ac9'];
    } else if (actionType === 'sick') {
      title = "AI Cold & Cough Recovery Pack";
      selectedIds = ['dolo-650-medic', 'cough-syrup', 'first-aid-kit'];
    } else if (actionType === 'guests') {
      title = "AI Host Party Snacks Combo";
      selectedIds = ['mock-choco-cookies', 'mock-choco-cake', 'b819f71c-32e6-42ee-8e7f-b649d2146ac9', 'c905db42-998f-4cf6-ba9e-1d5be5a248f2'];
    }

    const items = MOCK_PRODUCTS.filter(p => selectedIds.includes(p.id));
    setAiRecommendedCart(items);
    setAiOrderBuilderPrompt(title);
    setShowAiCartPopup(true);
    showToast(`${title} generated by AI!`, 'info');
  };

  const handleAiOrderBuilder = (e: React.FormEvent) => {
    e.preventDefault();
    if (!aiOrderBuilderPrompt.trim()) return;

    const lower = aiOrderBuilderPrompt.toLowerCase();
    let items: Product[] = [];

    if (lower.includes('dinner') || lower.includes('5') || lower.includes('1200') || lower.includes('people')) {
      // Build package under 1200 for 5 people
      items = MOCK_PRODUCTS.filter(p => 
        ['combo-family', 'b819f71c-32e6-42ee-8e7f-b649d2146ac9', 'mock-choco-cake', 'c905db42-998f-4cf6-ba9e-1d5be5a248f2'].includes(p.id)
      );
    } else {
    ...
    setAiRecommendedCart(items);
    setShowAiCartPopup(true);
    showToast(`AI compiled order for "${aiOrderBuilderPrompt}"!`, 'success');
  };

  const addAllAiItemsToCart = () => {
    if (!aiRecommendedCart) return;
    aiRecommendedCart.forEach(item => {
      addToCart(item);
    });
    setShowAiCartPopup(false);
    setAiRecommendedCart(null);
    showToast("Added all AI recommended items to your cart!", "success");
  };
  ...
  return (
    <div className={`flex flex-col gap-10 sm:gap-12 min-h-screen transition-all duration-700 ${
      weather === 'rainy' ? 'weather-rainy' : weather === 'winter' ? 'weather-winter' : weather === 'sunny' ? 'weather-sunny' : ''
    }`}>
      ...
      <div className="-mx-4 -mt-4 p-4 sm:-mx-8 sm:-mt-8 sm:p-6 bg-[#070B14] text-white flex flex-col gap-5 relative overflow-hidden pb-6 font-sans">
        ...
        <div className="relative z-50 flex flex-wrap gap-2 items-center justify-between p-2 rounded-2xl bg-white/5 border border-white/10 text-[10px]">
          <div className="flex gap-1.5 items-center">
            <span className="text-zinc-400 font-bold uppercase tracking-wider mr-1">🕒 Time:</span>
            {(['morning', 'afternoon', 'evening', 'night'] as const).map(t => (
              <button
                key={t}
                onClick={() => setTimeOfDay(t)}
                className={`px-2 py-1 rounded-lg capitalize font-bold transition-all ${timeOfDay === t ? 'bg-[#FF5F1F] text-white' : 'bg-white/5 text-zinc-300 hover:bg-white/10'}`}
              >
                {t}
              </button>
            ))}
          ...
          <div className="flex gap-1.5 items-center">
            <span className="text-zinc-400 font-bold uppercase tracking-wider mr-1">🌧️ Weather:</span>
            {(['sunny', 'rainy', 'winter'] as const).map(w => (
              <button
                key={w}
                onClick={() => setWeather(w)}
                className={`px-2 py-1 rounded-lg capitalize font-bold transition-all ${weather === w ? 'bg-primary text-white' : 'bg-white/5 text-zinc-300 hover:bg-white/10'}`}
              >
                {w}
              </button>
            ))}
          ...
          <div className="flex gap-1.5 items-center">
            <span className="text-zinc-400 font-bold uppercase tracking-wider mr-1">🛡️ Modes:</span>
            <button
              onClick={() => setStudentMode(!studentMode)}
              className={`px-2.5 py-1 rounded-lg font-bold transition-all flex items-center gap-1 ${studentMode ? 'bg-[#7C3AED] text-white' : 'bg-white/5 text-zinc-300'}`}
            >
              🎓 Student
            </button>
            <button
              onClick={() => setFamilyMode(!familyMode)}
              className={`px-2.5 py-1 rounded-lg font-bold transition-all flex items-center gap-1 ${familyMode ? 'bg-pink-500 text-white' : 'bg-white/5 text-zinc-300'}`}
            >
              👨‍👩‍👧‍👦 Family
            </button>
          </div>
        ...
        {!searchQuery.trim() && (
          <div className="w-full bg-[#101828]/95 border border-white/8 rounded-[2rem] p-5 flex flex-col gap-4 shadow-xl relative overflow-hidden backdrop-blur-xl select-none text-left">
            ...
            <div className="z-10 mt-1">
              <span className="text-[9px] font-black uppercase text-zinc-500 tracking-wider block mb-2">Try AI Shortcuts:</span>
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => handleAiAction('budget')}
                  className="bg-white/5 hover:bg-white/10 border border-white/8 rounded-xl px-3 py-2 text-[10px] font-bold text-zinc-200 flex items-center gap-1.5 transition-all active:scale-95 cursor-pointer"
                >
                  💰 "I have ₹250"
                </button>
                <button
                  onClick={() => handleAiAction('sick')}
                  className="bg-white/5 hover:bg-white/10 border border-white/8 rounded-xl px-3 py-2 text-[10px] font-bold text-zinc-200 flex items-center gap-1.5 transition-all active:scale-95 cursor-pointer"
                >
                  🤒 "I'm sick"
                </button>
                <button
                  onClick={() => handleAiAction('guests')}
                  className="bg-white/5 hover:bg-white/10 border border-white/8 rounded-xl px-3 py-2 text-[10px] font-bold text-zinc-200 flex items-center gap-1.5 transition-all active:scale-95 cursor-pointer"
                >
                  👥 "I have guests"
                </button>
            ...
            <form onSubmit={handleAiOrderBuilder} className="z-10 mt-1 w-full flex gap-2">
              <input
                ...
                onChange={(e) => setAiOrderBuilderPrompt(e.target.value)}
                className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-[#8B5CF6] transition-colors"
              />
              <button
                type="submit"
                className="bg-[#8B5CF6] hover:bg-[#7C3AED] text-white text-[10px] font-black uppercase tracking-wider px-4 rounded-xl transition-all active:scale-95 flex items-center justify-center cursor-pointer shrink-0"
              >
                Assemble
              </button>
            </form>
      ...
      {/* 🤖 AI Recommended Cart Popup Overlay */}
      {showAiCartPopup && aiRecommendedCart && (
        <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-background/80 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="relative w-full max-w-sm bg-[#101828]/95 border border-white/10 rounded-[2.5rem] p-6 shadow-2xl flex flex-col gap-5 text-left glass-panel">
            {/* Ambient Purple Glow */}
            <div className="absolute -right-12 -top-12 w-32 h-32 bg-[#8B5CF6]/15 rounded-full blur-[40px] pointer-events-none"></div>

            {/* Header */}
            <div className="flex justify-between items-start border-b border-white/5 pb-3">
              <div>
                <span className="text-[9px] font-black uppercase text-[#8B5CF6] tracking-widest block">Velto AI Compiler</span>
                <h3 className="text-sm font-black uppercase tracking-wider text-white mt-0.5">Recommended Package</h3>
              </div>
              <button 
                onClick={() => {
                  setShowAiCartPopup(false);
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
              {aiRecommendedCart.map((item) => (
                <div key={item.id} className="flex justify-between items-center bg-white/5 rounded-2xl p-3 border border-white/5">
                  <div className="flex flex-col text-left">
                    <span className="text-xs font-black text-white">{item.name}</span>
                    <span className="text-[9px] text-zinc-400 font-semibold">{item.category}</span>
                  </div>
                  <span className="text-xs font-black text-[#FF5F1F]">₹{item.price}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex justify-between items-center border-t border-white/5 pt-4">
              <span className="text-xs font-black uppercase text-zinc-400">Total Price</span>
              <span className="text-lg font-black text-white">
                ₹{aiRecommendedCart.reduce((acc, curr) => acc + curr.price, 0)}
              </span>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <button
                onClick={addAllAiItemsToCart}
                className="w-full bg-gradient-to-r from-[#8B5CF6] to-[#7C3AED] text-white text-xs font-black uppercase tracking-wider py-4 rounded-2xl hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-[#8B5CF6]/20 cursor-pointer text-center"
              >
                Order Everything (Add to Cart)
              </button>
              <button
                onClick={() => {
                  setShowAiCartPopup(false);
                ...
                className="w-full bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white text-[10px] font-black uppercase tracking-wider py-3.5 rounded-2xl transition-all cursor-pointer text-center"
              >
                Cancel
              </button>
            </div>
